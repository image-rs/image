...
fn check_image<P: AsRef<Path>>(c: Config, fname: P) -> io::Result<()> {
    ...
    use png::Decoded::*;
    let mut t = r#try!(term::stdout().ok_or(io::Error::new(
        io::ErrorKind::Other,
        "could not open terminal"
    )));
    let mut data = vec![0; 10*1024];
    let data_p = data.as_mut_ptr();
    let mut reader = io::BufReader::new(r#try!(File::open(&fname)));
    let fname = fname.as_ref().to_string_lossy();
    let n = r#try!(reader.read(&mut data));
    let mut buf = &data[..n];
    let mut pos = 0;
    ...
    let display_error = |err| -> Result<_, io::Error> {
        ...
        if c.verbose {
            if c.color {
                print!(": ");
                r#try!(t.fg(color::RED));
                r#try!(writeln!(t, "{}", err));
                r#try!(t.attr(Attr::Bold));
                r#try!(write!(t, "ERRORS DETECTED"));
                r#try!(t.reset());
            } else {
                println!(": {}", err);
            ...
            if !c.quiet { if c.color {
                ...
                r#try!(t.reset());
                r#try!(write!(t, ": "));
                r#try!(t.fg(color::YELLOW));
                r#try!(writeln!(t, "{}", fname));
                r#try!(t.reset());
            } else {
                println!("ERROR: {}", fname)
            }}
    ...
    loop {
        if buf.len() == 0 {
            ...
            let n = r#try!(reader.read(unsafe {
        ...
        match decoder.update(buf, &mut Vec::new()) {
            Ok((_, ImageEnd)) => {
                if !have_idat {
                    r#try!(display_error(png::DecodingError::Format("IDAT chunk missing".into())));
                    break;
                }
                if !c.verbose && !c.quiet {
                    if c.color {
                        r#try!(t.fg(color::GREEN));
                        r#try!(t.attr(Attr::Bold));
                        r#try!(write!(t, "OK"));
                        r#try!(t.reset());
                        r#try!(write!(t, ": "));
                    ...
                    if c.color {
                        r#try!(t.fg(color::GREEN));
                        r#try!(t.attr(Attr::Bold));
                        r#try!(write!(t, "No errors detected "));
                        r#try!(t.reset());
                    } else {
    ...
    if let Some(m) = parse_args() {
        ...
        for file in m.free {
            match if file.contains("*") {
                (|| -> io::Result<_> {
                    for entry in r#try!(glob::glob(&file).map_err(|err| {
                        io::Error::new(io::ErrorKind::Other, err.msg)
                    })) {
                        r#try!(check_image(config, r#try!(entry.map_err(|_| {
