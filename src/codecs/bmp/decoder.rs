...
/// Offsets within BITMAPV5HEADER (after the 4-byte header size field)
        ...
        loop {
            ...
            match state {
                ...
                DecoderState::ReadingDibHeader {
                    ...
                    // Parse the DIB header from buffer directly
                    match self.bmp_header_type {
                        BMPHeaderType::Core => {
                            // BITMAPCOREHEADER: width(2), height(2), planes(2), bitcount(2) = 8 bytes
                            if buffer.len() < 8 {
                                return Err(ImageError::Decoding(DecodingError::new(
                                    ImageFormat::Bmp.into(),
                                    "Insufficient data for core header",
                                )));
                            }
                        ...
                        _ => {
                            ...
                            // width(4), height(4), planes(2), bitcount(2), compression(4), sizeimage(4),
                            // xpelspermeter(4), ypelspermeter(4), clrused(4), clrimportant(4) = 36 bytes minimum
                            if buffer.len() < 36 {
                                return Err(ImageError::Decoding(DecodingError::new(
                                    ImageFormat::Bmp.into(),
                                    "Insufficient data for info header",
                                )));
                            }
                            ...
                            if self.width < 0 {
                                return Err(DecoderError::NegativeWidth(self.width).into());
                            } else if self.width > MAX_WIDTH_HEIGHT
                                || self.height > MAX_WIDTH_HEIGHT
                            {
                                return Err(
                                    DecoderError::ImageTooLarge(self.width, self.height).into()
                                );
                            }
                            ...
                            let image_type_u32 = u32::from_le_bytes([
                                buffer[12], buffer[13], buffer[14], buffer[15],
                            ]);

                            // Top-down dibs cannot be compressed
                            if self.top_down && image_type_u32 != 0 && image_type_u32 != 3 {
                                return Err(DecoderError::ImageTypeInvalidForTopDown(
                                    image_type_u32,
                                )
                                .into());
                            }
                            ...
                            self.image_type = match image_type_u32 {
                                ...
                                _ => {
                                    return Err(
                                        DecoderError::ImageTypeUnknown(image_type_u32).into()
                                    );
                                }
                            };

                            // Skip image size (bytes 16-19), xpelspermeter (20-23), ypelspermeter (24-27)
                            self.colors_used = u32::from_le_bytes([
                                buffer[28], buffer[29], buffer[30], buffer[31],
                            ]);
                            // Skip clrimportant (bytes 32-35)
                        }
                    }

                    // Check if we need to store ICC profile metadata (V5 header)
                    if header_size >= BITMAPV5HEADER_SIZE {
                        // Extract ICC profile metadata from V5 header
                        if buffer.len() >= (BITMAPV5HEADER_SIZE - 4) as usize {
                            ...
                            if cs_type == PROFILE_EMBEDDED {
                                ...
                                if profile_size > 0 && profile_offset > 0 {
                                    self.icc_profile_offset = Some(u64::from(profile_offset));
                                    self.icc_profile_size = Some(profile_size);
                                }
                            }
                        }
                    }
            ...
            if control_byte == RLE_ESCAPE {
                ...
                match op {
                    RLE_ESCAPE_EOL => {
                        ...
                        row_buffer[(x_position * num_channels)..row_byte_length].fill(0);
