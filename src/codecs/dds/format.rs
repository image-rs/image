use super::{
    decoder::SupportedFormat,
    header::{BitFlags, DxgiFormat, Header, PixelFormat, PixelFormatFlags},
};

/// Returns the format of the surfaces from a DDS header.
pub(crate) fn detect_format(header: &Header) -> Option<SupportedFormat> {
    if let Some(dx10_header) = header.dx10 {
