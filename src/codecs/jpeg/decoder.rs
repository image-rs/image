...
impl<R: Reader>JPEGDecoder<R> {
    ...
    fn decode_mcu_row(&mut self) -> ImageResult<()> {
        ...
        for x0 in range_step(0, self.padded_width * bytesperpixel,
            bytesperpixel * 8 * self.hmax as uint) {
    ...
    fn decode_mcu(&mut self) -> ImageResult<()> {
        ...
        for id in tmp.iter() {
            ...
            for _ in range(0, c.h * c.v) {
                let pred  = try!(
                    self.decode_block(i, c.dc_table, c.dc_pred, c.ac_table, c.tq));
                c.dc_pred = pred;
