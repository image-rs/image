...
impl<W: Writer> JPEGEncoder<W> {
    ...
    pub fn encode(&mut self,
        ...
        let buf = build_frame_header(8, width as u16, height as u16,
            self.components.slice_to(num_components));
        ...
        for y in range_step(0, height, 8) {
            for x in range_step(0, width, 8) {
                ...
                for i in range(0u, 64) {
                    dct_yblock[i] = (
                        (dct_yblock[i] / 8) as f32 / self.tables.slice_to(64)[i] as f32)
                        .round() as i32;
                }
                ...
                y_dcprev  = try!(
                    self.write_block(dct_yblock, y_dcprev, ld.as_slice(), la.as_slice()));
            }
        }
            ...
            for x in range_step(0, width, 8) {
                ...
                for i in range(0u, 64) {
                    dct_yblock[i]   = (
                        ...
                        .round() as i32;
                    dct_cb_block[i] = (
                        (dct_cb_block[i] / 8) as f32 / self.tables.slice_from(64)[i] as f32)
                        .round() as i32;
                    dct_cr_block[i] = (
                        (dct_cr_block[i] / 8) as f32 / self.tables.slice_from(64)[i] as f32)
                        .round() as i32;
                }
                ...
                y_dcprev  = try!(
                    self.write_block(dct_yblock, y_dcprev, ld.as_slice(), la.as_slice()));
                cb_dcprev = try!(
                    self.write_block(dct_cb_block, cb_dcprev, cd.as_slice(), ca.as_slice()));
                cr_dcprev = try!(
                    self.write_block(dct_cr_block, cr_dcprev, cd.as_slice(), ca.as_slice()));
            }
        }
