...
pub fn filter(method: FilterType, bpp: uint, previous: &[u8], current: &mut [u8]) {
    ...
    match method {
        ...
        Avg  => {
            ...
            for i in range(bpp, len) {
                current[i] = orig.as_slice()[i] - (
                    (orig.as_slice()[i - bpp] as i16 + previous[i] as i16) / 2) as u8;
            }
        }
        Paeth    => {
            for i in range(0, bpp) {
            ...
            for i in range(bpp, len) {
                current[i] = orig.as_slice()[i] - filter_paeth(
                    orig.as_slice()[i - bpp], previous[i], previous[i - bpp]);
            }
        }
    }
}