...
/// A limit on the maximum length of the TUPLTYPE field in a PAM image
/// that the PnmDecoder will accept. Unlimited TUPLTYPE lengths could
/// require unlimited allocation, so set an arbitrary limit which is
/// more than the standard TUPLTYPE name lengths.
const MAX_TUPLE_TYPE_LENGTH: usize = 64;
...
enum DecoderError {
    ...
    /// A header line had an ID that was too long
    HeaderLineIDTooLong,
...
impl Display for DecoderError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            DecoderError::HeaderLineIDTooLong => f.write_fmt(format_args!(
                "Unknown header line with overly long identifier"
            )),
            ...
            DecoderError::TupleTypeTooLong => f.write_str("The PAM tuple type name was too long"),
            ...
            match byte[0] {
                ...
                b => {
                    if id_len >= identifier.len() {
                        return Err(DecoderError::HeaderLineIDTooLong.into());
                    }
                    identifier[id_len] = b;
                    id_len += 1;
                }
...
ENDHDR
";
        let DecoderError::HeaderLineUnknown(h) = pam_header_decode_error(header) else {
...
";
        ...
        assert!(matches!(
            pam_header_decode_error(header.as_bytes()),
            DecoderError::HeaderLineIDTooLong
        ));
    }
