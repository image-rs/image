        ...
        match *self.unchecked.header {
                ...
                _ => {
                    return Err(ImageError::Parameter(ParameterError::from_kind(
                        ParameterErrorKind::Generic(format!(
                            "PBM format only support luma color types, not {:?}",
                            color
                        )),
                    )))
                }
            ...
            PnmHeader {
                ...
                _ => {
                    return Err(ImageError::Parameter(ParameterError::from_kind(
                        ParameterErrorKind::Generic(format!(
                            "PGM format only support luma color types, not {:?}",
                            color
                        )),
                    )))
                }
            ...
            PnmHeader {
                ...
                _ => {
                    return Err(ImageError::Parameter(ParameterError::from_kind(
                        ParameterErrorKind::Generic(format!(
                            "PPM format only support Rgb8 or Rgb16, not {:?}",
                            color
                        )),
                    )))
                }
