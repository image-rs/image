    ...
    fn loop_filter(&mut self, mbx: usize, mby: usize, mb: &MacroBlock) {
        if self.frame.filter != 0 {
            ...
            if segment.loopfilter_level != 0 {
                ...
                let (filter_level, interior_limit, hev_threshold) = self.calculate_filter_parameters(mb, segment);
