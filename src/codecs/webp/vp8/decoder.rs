...
pub struct Frame {
    ...
    /// The u plane of the frame(blue projection)
    pub ubuf: Vec<u8>,
    /// The v plane of the frame(red projection)
    pub vbuf: Vec<u8>,
...
impl<R: Read> Vp8Decoder<R> {
    ...
    //mbx and mby are the x and y indices of the macroblock mb
    //resdata is the residual data for fixing up luma
    ...
    fn intra_predict_chroma(&mut self, mbx: usize, mby: usize, mb: &MacroBlock, resdata: &[i32]) {
        ...
        for y in 0usize..stride-1 {
            let (uy, vy) = if mbx == 0 {
                ...
                if index >= self.frame.ubuf.len() {
                    //println!("y over: mbx: {}, mby: {}, y: {}", mbx, mby, y);
                    (129, 129)
                } else {
                    (
        ...
        for x in 0usize..stride-1 {
            let (ux, vx) = if mby == 0 {
                ...
                if index >= self.frame.ubuf.len() {
                    //println!("x over: mbx: {}, mby: {}, x: {}", mbx, mby, x);
                    (127, 127)
                } else {
                    (
                        self.frame.ubuf[index],
        ...
        //loop over 2 4x4 blocks in macroblock
        for y in 0usize..2 {
            for x in 0usize..2 {
                let i = x + y * 2;
                let mut urb = [0i32; 16];
